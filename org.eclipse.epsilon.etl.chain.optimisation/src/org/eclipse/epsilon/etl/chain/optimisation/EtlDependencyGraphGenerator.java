...
public class EtlDependencyGraphGenerator implements IEtlVisitor {
	...
	HashMap<String , Region> lineNumber = new HashMap<>();
	@Override
...
//				for(Parameter p : rule.getTargetParameters())
//				staticAnalyser.setPossibleType(operationCallExpression, staticAnalyser.getType(p) );
			}
			}
			...
			for (TransformationRule rule : rules) {
				this.lineNumber.put(this.tr.getName()+","+rule.getName(), operationCallExpression.getRegion());
				markTraceable(rule);
			}
		...
		// Case 4
			...
			if (operationCallExpression.getParameterExpressions().isEmpty())
				...
				while (pi.hasNext()) {
					String ruleName = ((StringLiteral) pi.next()).getValue();
					rules.add(getTransformationRuleByName(ruleName));
//							operationCallExpression.getParent().getChildren().remove(operationCallExpression);
...
//							operationCallExpression.getParent().getChildren().add(op);
				}
			}
			for (TransformationRule rule : rules) {
				this.lineNumber.put(this.tr.getName()+","+rule.getName(), operationCallExpression.getRegion());
	...
	public void optimiseTrace(IEtlModule module, EtlStaticAnalyser staticAnalyser) {
		EtlModule etlModule = (EtlModule) module;
		this.module = etlModule;
		this.staticAnalyser = staticAnalyser;
//		dependencyGraph = new DefaultDirectedGraph<TransformationRule, DefaultEdge>(DefaultEdge.class);
		for (TransformationRule rule : etlModule.getTransformationRules()) {
//			dependencyGraph.addVertex(rule);
			this.trace.put(rule.getName(), new ArrayList<String>());
			rule.getData().put("transformed", false);
			rule.getData().put("needTraceOf", new ArrayList<>());
		...
		for (TransformationRule rule : etlModule.getTransformationRules()) {
			rule.getData().put("traceableBy", trace.get(rule.getName()));
			for (String r : trace.get(rule.getName())) {
				((List) getTransformationRuleByName(r).getData().get("needTraceOf")).add(rule);
			}
		}
		exportLineNumber();
//		int i = module.getSourceUri().getPath().lastIndexOf('.');
//		  String name = module.getSourceUri().getPath().substring(0,i);
////		exportDependencyGraphToDot(name+ ".dot");
//		reOrderTransformationRules(module);
...
//	private void reOrderTransformationRules(IEtlModule module) {
//		module.getTransformationRules().clear();
//		TransitiveReduction.INSTANCE.reduce(dependencyGraph);
//		TopologicalOrderIterator<TransformationRule, DefaultEdge> topologicalOrderIterator = new TopologicalOrderIterator<TransformationRule, DefaultEdge>(
//				new EdgeReversedGraph<>(this.dependencyGraph));
//		System.out.println("topologicalOrderIterator");
//		while (topologicalOrderIterator.hasNext()) {
//
//			module.getTransformationRules().add(topologicalOrderIterator.next());
//
//		}
//
//	}
...
//		if (!(dependencyGraph.containsEdge(rule, this.tr)) && !this.tr.equals(rule))
//			dependencyGraph.addEdge(this.tr, rule, new RelationEdge(""));
		 //Warning of cycles
		if(this.tr.equals(rule)) {
			System.err.println("I have a cyclic dependency");
			staticAnalyser.getContext().addErrorMarker(rule, "Cycle");
		}
	...
	public HashMap<String, Region> exportLineNumber() {
		return lineNumber;
	}

//	public void exportDependencyGraphToDot(String fileAndPath) {
//		DOTExporter<TransformationRule, DefaultEdge> exporter = new DOTExporter<>(
//				v -> replaceSymbolsForGraphviz(v.toString()));
...
//
//		exporter.exportGraph(dependencyGraph, writer);
//		exporter.exportGraph(dependencyGraph, new File(fileAndPath));
